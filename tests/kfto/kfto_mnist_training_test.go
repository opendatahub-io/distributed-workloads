...
func runKFTOPyTorchMnistJob(t *testing.T, totalNumGpus int, workerReplicas int, numCPUsOrGPUsCountPerNode int, gpuLabel string, image string, requirementsFile string) {
	...
	tuningJob := createKFTOPyTorchMnistJob(test, namespace.Name, *config, gpuLabel, totalNumGpus, workerReplicas, numCPUsOrGPUsCountPerNode, outputPvc.Name, image)
...
func createKFTOPyTorchMnistJob(test Test, namespace string, config corev1.ConfigMap, gpuLabel string, totalNumGpus int, workerReplicas int, numCPUsOrGPUsCountPerNode int, outputPvcName string, baseImage string) *kftov1.PyTorchJob {
	var useGPU = false
	var backend string
							...
							Containers: []corev1.Container{
								{
									...
									Command: []string{
										...
										torchrun --nproc_per_node=%d --nnodes=$(PET_NNODES) --node_rank=$(PET_NODE_RANK) --master_addr=$(MASTER_ADDR) --master_port=$(MASTER_PORT) \
										/mnt/files/mnist.py --epochs 7 --save_every 2 --batch_size 64 --lr 0.0005 --snapshot_path "mnist_snapshot.pt" --backend %s`, numCPUsOrGPUsCountPerNode, backend),
									},
									...
									Resources: corev1.ResourceRequirements{
										Limits: corev1.ResourceList{
											corev1.ResourceCPU:    resource.MustParse(fmt.Sprintf("%d", numCPUsOrGPUsCountPerNode)),
	...
	if useGPU {
		...
		tuningJob.Spec.PyTorchReplicaSpecs[kftov1.PyTorchJobReplicaTypeMaster].Template.Spec.Containers[0].Command = []string{
			...
			fmt.Sprintf(`mkdir -p /tmp/lib && export PYTHONPATH=$PYTHONPATH:/tmp/lib && \
			pip install --no-cache-dir -r /mnt/files/requirements.txt --target=/tmp/lib && \
			torchrun --nproc_per_node=%d --nnodes=%d --node_rank=$(PET_NODE_RANK) --master_addr=$(MASTER_ADDR) --master_port=$(MASTER_PORT) \
			/mnt/files/mnist.py --epochs 7 --save_every 2 --batch_size 128 --lr 0.001 --snapshot_path "mnist_snapshot.pt" --backend %s`, numCPUsOrGPUsCountPerNode, workerReplicas+1, backend),
		}
