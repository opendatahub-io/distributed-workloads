...
func runKFTOPyTorchMnistJob(test Test, accelerator Accelerator, image string, requirementsFile string, workerReplicas, numProcPerNode int) {
...
func createKFTOPyTorchMnistJob(test Test, namespace string, config corev1.ConfigMap, accelerator Accelerator, workerReplicas int, numProcPerNode int, baseImage string, localQueue *v1beta2.LocalQueue) *kftov1.PyTorchJob {
	var backend string
	if accelerator.IsGpu() {
		backend = "nccl"
	} else {
