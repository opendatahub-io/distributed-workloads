...
const (
	loraSftNotebookName = "lora_sft.ipynb"
	loraSftNotebookPath = "resources/" + loraSftNotebookName
)

// Multi-GPU - Distributed Training with LORA-SFT and TrainingHubTrainer
func RunLoraSftTrainingHubMultiGpuDistributedTraining(t *testing.T) {
	test := support.With(t)
	...
	localPath := loraSftNotebookPath
	nb, err := os.ReadFile(localPath)
	test.Expect(err).NotTo(HaveOccurred(), fmt.Sprintf("failed to read notebook: %s", localPath))
	cm := support.CreateConfigMap(test, namespace.Name, map[string][]byte{loraSftNotebookName: nb})
	...
	prefix, _ := support.GetStorageBucketLoraSftDir()
	if !endpointOK {
		endpoint = ""
	}
	if !bucketOK {
	...
	shellCmd := fmt.Sprintf(
		"set -e; "+
			...
			"export AWS_STORAGE_BUCKET_LORA_SFT_DIR='%s'; "+
	...
	common.CreateNotebook(test, namespace, userToken, command, cm.Name, loraSftNotebookName, 0, rwxPvc, common.ContainerSizeMedium)
