...
// Multi-GPU - Distributed Training with LoRA callbacks and TrainingHubTrainer
func RunLoraCallbacksTrainingHubMultiGpuDistributedTraining(t *testing.T, nnodes int) {
	test := support.With(t)
	...
	localPath := loraCallbacksNotebookPath
	...
	shellCmd := fmt.Sprintf(
		...
		loraCallbacksNotebookName,
	)
	command := []string{"/bin/sh", "-c", shellCmd}

	common.CreateNotebook(test, namespace, userToken, command, cm.Name, loraCallbacksNotebookName, 0, rwxPvc, common.ContainerSizeMedium, common.GetRecommendedNotebookImageFromImageStream(test, common.NotebookImageStreamTrainingHubCUDA))
