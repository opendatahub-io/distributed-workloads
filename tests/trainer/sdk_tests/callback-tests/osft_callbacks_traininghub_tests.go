...
// Multi-GPU - Distributed Training with OSFT callbacks and TrainingHubTrainer
func RunOsftCallbacksTrainingHubMultiGpuDistributedTraining(t *testing.T, nnodes int) {
	test := support.With(t)
	...
	localPath := osftCallbacksNotebookPath
	...
	shellCmd := fmt.Sprintf(
		...
		osftCallbacksNotebookName,
	)
	command := []string{"/bin/sh", "-c", shellCmd}

	common.CreateNotebook(test, namespace, userToken, command, cm.Name, osftCallbacksNotebookName, 0, rwxPvc, common.ContainerSizeMedium, common.GetRecommendedNotebookImageFromImageStream(test, common.NotebookImageStreamTrainingHubCUDA))
