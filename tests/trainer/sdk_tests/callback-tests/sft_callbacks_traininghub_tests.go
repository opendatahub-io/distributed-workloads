...
// Multi-GPU - Distributed Training with SFT callbacks and TrainingHubTrainer
func RunSftCallbacksTrainingHubMultiGpuDistributedTraining(t *testing.T, nnodes int) {
	test := support.With(t)
	...
	localPath := sftCallbacksNotebookPath
	...
	shellCmd := fmt.Sprintf(
		...
		sftCallbacksNotebookName,
	)
	command := []string{"/bin/sh", "-c", shellCmd}

	common.CreateNotebook(test, namespace, userToken, command, cm.Name, sftCallbacksNotebookName, 0, rwxPvc, common.ContainerSizeMedium, common.GetRecommendedNotebookImageFromImageStream(test, common.NotebookImageStreamTrainingHubCUDA))
