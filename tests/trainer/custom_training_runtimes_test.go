...
var expectedRuntimeNames = []string{
	"torch-cuda-241",
	"torch-cuda-251",
	"torch-rocm-241",
	"torch-rocm-251",
...
func TestCustomTrainingRuntimesAvailable(t *testing.T) {
	...
	for _, expectedName := range expectedRuntimeNames {
		test.Expect(foundRuntimeNames[expectedName]).To(BeTrue(),
			fmt.Sprintf("Expected ClusterTrainingRuntime '%s' not found", expectedName))
		test.T().Logf("Found expected runtime: %s", expectedName)
	}
