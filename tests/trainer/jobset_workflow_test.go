...
func TestJobSetWorkflow(t *testing.T) {
	...
	// Verify JobSet is created with 3 replicated jobs
	test.Eventually(func() error {
		jobset, err := GetSingleJobSet(test, namespace)
		if err != nil {
			return err
		}
		...
		if !found {
			return fmt.Errorf("JobSet spec.replicatedJobs not found")
		}
		if len(replicatedJobs) != 3 {
			return fmt.Errorf("expected 3 replicated jobs, got %d", len(replicatedJobs))
		}
		return nil
	}, TestTimeoutShort).Should(Succeed())
	test.T().Log("JobSet is created with 3 replicated jobs (dataset-initializer, model-initializer, node)")
...
func TestFailedJobSetWorkflow(t *testing.T) {
	...
	// Wait for JobSet to fail and verify the failure message
	...
	test.Eventually(func() error {
		...
		conditions, found, err := unstructured.NestedSlice(jobset.Object, "status", "conditions")
		if err != nil {
			return err
		}
		if !found {
			return fmt.Errorf("JobSet status.conditions not found")
		}
		...
		for _, cond := range conditions {
			...
			if condType == "Failed" {
				...
				if status == "True" {
					failureMessage, found, err = unstructured.NestedString(condMap, "message")
					if err != nil {
						return err
					}
					if found {
		...
		return fmt.Errorf("JobSet failure condition not found")
	}, TestTimeoutMedium).Should(Succeed())
	test.T().Logf("JobSet failed with message: %s", failureMessage)
											...
											Containers: []corev1.Container{
												{
													...
													Args: []string{
														...
														echo "Copying Alpaca dataset to shared volume..."
											...
											Containers: []corev1.Container{
												{
													...
													Args: []string{
														...
														echo "Copying Bloom-560M model to shared volume ..."
														mkdir -p ${MODEL_PATH}
														cp -r /models/bloom-560m ${MODEL_PATH}/
														ls -la ${MODEL_PATH}/
														echo ""
...
func createTrainJobWithFailingInitializer(test Test, namespace, runtimeName string) *trainerv1alpha1.TrainJob {
	...
	test.Expect(err).NotTo(HaveOccurred(), "Failed to create TrainJob")
	test.T().Logf("Created TrainJob %s/%s (with failing initializer)", createdTrainJob.Namespace, createdTrainJob.Name)
